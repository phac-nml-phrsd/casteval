...
test_that("widen_NULL() accepts empty list", {
  expect_equal(widen_NULL(list(), 1), list())
})

test_that("widen_NULL() widens to 1", {
  expect_equal(
    widen_NULL(list(NULL, 1, NULL, 2, NULL, 3), 1),
...
test_that("widen_NULL() widens to multiple", {
  expect_equal(
    widen_NULL(list(NULL, c(1,2,3), NULL), 3),
...
test_that("widen_NULL() widens to 0", {
  expect_equal(
    widen_NULL(list(NULL, 1, c(2,3,4), NULL), 0),
...
test_that("widen_NULL() does not modify non-NULL", {
  expect_equal(
    widen_NULL(list(list(), NA, logical(0), numeric(0), NA_real_, 1, c(1,2,3), "hi", 0, FALSE), 2),
...
test_that("combine_two_data_frames() demands non-empty data frames", {
  expect_error(
    combine_two_data_frames(data.frame(), data.frame(a=1)),
    "nrow.*df1"
  )
  expect_error(
    combine_two_data_frames(data.frame(a=1), data.frame()),
    "nrow.*df2"
  )
})
...
test_that("combine_data_frames() accepts 1 data frame", {
  expect_equal(
    combine_data_frames(list(data.frame(time=c(1,2,3), raw=c(4,5,6)))),
