...
Internally, `{casteval}` works with what are called "forecast data frames".
All forecast data frames contain a `time` column, which can contain either numbers, dates, or datetimes.
In addition, they must contain either unsummarized or summarized forecast data.

Unsummarized data is stored in the `val` column.
A `sim` column may also be provided to 

Summarized data is stored in the following columns:
