...
vignette: >
  %\VignetteIndexEntry{Evaluate time series forecasts with casteval}
...
This vignette guides you through this process.^[In this vignette, we demonstrate `{casteval}`'s main functionality via deliberately simple examples. For a more realistic start-to-finish demonstration of the package's usage with real forecasts, please see [`vignette("denmark2020")`](denmark2020.html).]
...
### Grouping
`{casteval}` supports *grouping*, whereby multiple locations, scenarios, variables, etc. can be stored in a single data frame for scoring and plotting. For details, see this [vignette](grouping.html)
