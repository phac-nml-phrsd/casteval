...
`plot_forecast(fcst, obs=NULL, quant_pairs=NULL, invert_scale=FALSE, score=NULL)` plots a forecast and optionally scores a forecast, overlays observations, quantile intervals, and color-codes the observations based on score. 

For raw forecast data, `quant_pairs` is like the parameter with the same name in `accuracy()`, though here it is only used to draw quantile bands on the plot (and not in the scoring).
...
They are as follows:^[We have omitted some optional parameters below for brevity. See the function documentation for details.]
...
(plot_ensemble(fcst=fc) 
  ...
  |> plot_observations(accuracy(fc, obs, quant_pairs=c(25,75), summarize=FALSE))
)
```