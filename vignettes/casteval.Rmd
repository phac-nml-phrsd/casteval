...
To make the use of scoring functions easier, we provide the following function factories:

- `make_accuracy(quant_pairs=NULL)` creates a function wrapping `accuracy()` with the given quantile pairs
- `make_log_score(at=NULL, after=NULL, bw=NULL)` creates a function wrapping `log_score()` with the given parameters
...
# use make_log_accuracy() to set binwidth
plot_forecast(fc, obs, score=make_log_score(bw=0.5))
```
