...
In the context of this package, a "forecast data frame" is a data frame which has:

- a `time` column, containing the time
- (optional) a `sim` column, containing the simulation number (a.k.a. the realization number)
- (optional) any number of columns that start with `grp_`, e.x. `grp_scenario`, `grp_province`, `grp_variable`, etc.
- (optional) a `val` column, containing raw forecast values
- (optional) any number of columns that start with `val_`, e.x. `val_q2.5`, `val_mean`

`sim` is required for any curve-based scoring methods, as well as for plotting of raw ensemble curves.
It is not required for fixe-time scoring methods.

Columns starting with `grp_` will be used to group the data before scoring and plotting.

Columns starting with `val_` contain summary data (`val` itself contains unsummarized raw data) such as quantiles (e.x. `val_q50` for the median) and the mean.

# Input formatting

At a high level, a time series forecast is specified with two pieces of information: forecasted values and the accompanying time at which the forecasted values are predicted to be observed. In practice, forecasts can be presented in various ways. A forecast may be given as many individual realizations of a model, or this ensemble may be summarized into a mean and confidence interval for each predicted time point. Additionally, a forecast may be produced for a single scenario (_e.g._ a simple, status quo forecast) or several, distinct scenarios may be considered (status quo alongside the introduction of an intervention with varying degrees of success in controlling an outbreak). 

With these applications in mind, `{casteval}` accepts several input formats, including:

- A single data frame with a [`time` column](#input-time) and one or more [forecast data columns](#input-forecast-data) 
- A list of data frames, each with a `time` column and a [`raw` data column](#input-forecast-data)
- A named list `l`, where `l$time` is a vector of times and `l$ensemble` is a list of vectors containing forecast data, each one the same length as `l$time`
...
## Forecast data {#input-forecast-data}

All forecast data columns must contain numeric data.
The current supported forecast data columns are:
...
The `interval` parameter should be a vector of two percentages from 0 to 100 (defaults to `c(2.5, 97.5)`). The corresponding quantiles will be calculated from the raw data and/or provided quantile columns in order to calculate the score.
...
fc3 <- create_forecast(dplyr::tibble(
    time=1:5,
    quant_2.5=c(6,6,6,6,6),
    quant_97.5=c(10,10,10,10,10)
))

obs <- data.frame(time=1:5, obs=c(0, 2.4, 5, 9.5, 10))

# Calculate accuracy from raw data (50% confidence interval)
accuracy(fc1, obs, interval=c(25, 75))

# Calculate accuracy using provided quantiles (90% confidence interval)
accuracy(fc2, obs, interval=c(5, 95))
