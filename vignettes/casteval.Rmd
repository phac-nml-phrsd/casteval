...
waldo::compare(arrange(fc1$data, time), fc2$data)
...
`fcst` must contain unsummarized data with 2 or more realizations per time point.
...
If `summarize` is FALSE, then `at` and `after` are ignored, and the output will be as described above.
...
### Visualizing the KDE
When evaluating forecasts using `log_score()` you may want to inspect inspect KDE as a sanity check, especially if your data is very sparse.
You may also want to see how setting the `bw` parameter affects the resulting distribution, or whether the automatically calculated binwidth is good enough.

`plot_KDE()` allows you to do this.
Like `log_score()`, it accepts the parameters `fcst`, `obs` (which is now optional), `at`, `after`, and `bw` (it does not have a `summarize` flag).
It also accepts the optional parameters `from`, `to`, `n`, and `binwidth` (not to be confused with bandwidth). See `?plot_KDE` for a detailed explanation of these parameters.

`plod_KDE()` will plot the following elements:
...
If it implements the `summarize` flag, then it can be used in plotting functions as well.
...
`quant_pairs` is the same as the parameter for `accuracy()`.
The only difference is that an error will not be raised if `quant_pairs` is `NULL` and quantile pairs can't be inferred, or if `quant_pairs` is an empty list (in which case quantile intervals simply won't be displayed).
...
# Identify the observations outside the quantile interval
