...
If you need to pass additional arguments to a scoring function, it needs to be wrapped so that it only accepts `fcts`, `obs`, and `summarize`. For example, `accuracy()` needs a quantile pair specified, so we can wrap it as follows:

```{r plot_forecast_examples2}
# We use a function factory to wrap accuracy() (see `?make_accuracy`)
quant <- c(5,95)
acc <- make_accuracy(quant)

# Identify the observations inside the 5%-95% quantile interval
pp <- plot_forecast(fc, obs=obs, quant_pairs=list(c(25,75), c(2.5,97.5), c(5,95)), score=acc)
pp
```

Note that since `plot_forecast()` returns a `ggplot2::ggplot` object, you can further customize the plot to suit your application:
