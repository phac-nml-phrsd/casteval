...
`plot_forecast()` plots a forecast and optionally overlays observations, quantile intervals, and color-codes the observations based on score. 

Its `quant_intervals` parameter is like the `quant_pairs` parameter `accuracy()`, and it is used to draw quantile intervals on the plot.^[It is named `quant_intervals` and not `quant_pairs` so that it does not conflict when passing `quant_pairs` through the `...` parameter to a scoring function]
...
NULL |> plot_ensemble(fc) |> plot_mean(fc) |> plot_quant_intervals(fc, c(25,75)) |> plot_obs_score(fc, obs, score=accuracy, quant_pairs=c(25,75))
```