...
`crps(fcst, obs, at=NULL, after=NULL, summarize=TRUE)` is the [Continuous Ranked Probability Score](https://en.wikipedia.org/wiki/Scoring_rule#Continuous_ranked_probability_score). It is similar to the `log_score()` in that it can only be performed on raw forecast data, since it requires a distribution of predicted observations at each fixed time. 
...
It is also penalises inaccurate forecasts less harshly than `log_score()`.
